...
-- Policies
-- Batches: HO can manage all, branch managers can manage/select own branch
create policy if not exists "HO can manage production batches"
  on public.production_batches
  for all
  using (get_current_user_role() = 'ho_admin');

create policy if not exists "Branch managers manage own branch batches"
  on public.production_batches
  for all
...
-- Items follow their parent batch access
create policy if not exists "HO can manage production items"
  on public.production_items
  for all
  using (get_current_user_role() = 'ho_admin');

create policy if not exists "Branch managers manage own branch items"
  on public.production_items
  for all
  using (
